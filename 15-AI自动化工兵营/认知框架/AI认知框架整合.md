---
title: AI认知框架整合
topic: ''
created: '2026-05-17'
updated: '2026-05-17'
type: merged_knowledge_node
status: active
quality_level: B
summary_cn: AI认知框架与方法论整合：涵盖TDD工作流、Karpathy认知框架、学习Pack模板、安全边际、心智定位、客户痛点诊断等核心认知文档。用于快速查找AI工作流与学习方法论。
summary_en: 'AI cognitive frameworks and methodologies integration: covering TDD workflow,
  Karpathy framework, Learning Pack templates, safety margin, mental positioning,
  customer pain diagnosis and other core cognitive documents.'
one_sentence_summary: '# AI认知框架整合  ## 一句话总结  AI认知框架与方法论：TDD工作流、Karpathy认知、学习Pack、安全...'
decision_value: AI tool usage and prompt engineering reference
review_note: AI tool reference
category: ''
platform: []
industry: []
scenario: []
source_files: '|'
merged_from: 27个认知框架与方法论文档整合（不含TDD工作流与最佳实践整合.md）
old_versions: '|'
archived_reason: []
relations: []
related:
  - [[01-全库工作总览MOC]]
retrieval_keywords:
- 知识整合
- 最佳实践
- 认知框架整合
- 需求文档
- 销售方法论
- 安全边际
- 我的风险控制原则
- 学习
- 能力圈
- 版本与日期
routing_target:
  - "[[14-AI工具自动化MOC]]"
model_route:
  - "claude-code"
  - "openclaw"

---

# AI认知框架整合

## 一句话总结

AI认知框架与方法论：TDD工作流、Karpathy认知、学习Pack、安全边际、心智定位等27个方法论文档。

---

## 核心框架

### 1. TDD工作流

- TDD工作流与最佳实践整合.md - TDD完整方法论
- TDD Workflow.md, TDD-README.md, TDD-SKILL.md, TDD-最佳实践.md

### 2. Karpathy认知框架

- Karpathy认知框架.md

### 3. Learning Pack

- Learning Pack - SKILL 定义.md
- Learning Pack - 安装与使用指南.md
- Learning Pack - 需求文档.md

### 4. 商业认知

- 安全边际.md
- 心智定位.md
- 客户痛点诊断.md
- 客户问题即内容.md
- 工业品内容复利.md

### 5. 销售方法论

- 顾问式销售.md
- 高价值Offer.md
- 社交货币.md
- 视频证明大于文字承诺.md

### 6. 原则体系

- 我的复盘更新原则.md
- 我的成交原则.md
- 我的报价原则.md
- 我的跟进原则.md
- 我的长期主义原则.md
- 我的风险控制原则.md
- 简单具体可信.md
- 能力圈.md

---

## 版本与日期

- 整合版本：2026-05-17
- 来源：27个认知框架方法论文档
- 分类：AI工作流/认知框架

---

*本整合文稿由 iMac 主控 Agent 生成 | 2026-05-17*